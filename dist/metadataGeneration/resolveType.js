...
function getEnumerateType(typeNode) {
    ...
    return {
        enumMembers: enumDeclaration.members.map(function (member, index) {
            var value = metadataGenerator_1.MetadataGenerator.current.typeChecker.getConstantValue(member);
            if (value !== null && value !== undefined) {
                return value;
            }
            return getEnumValue(member) || index;
        }),
        enumNames: enumDeclaration.members.map(function (member, index) {
...
exports.getModelTypeDeclaration = getModelTypeDeclaration;
function getModelTypeProperties(node, genericTypes) {
    if (node.kind === ts.SyntaxKind.InterfaceDeclaration) {
        ...
        return interfaceDeclaration.members
            ...
            var type = !isGenericArray ? ResolveType(aType) : {
